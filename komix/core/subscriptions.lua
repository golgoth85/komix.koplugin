...
function Subscriptions:new(plugin)
    return setmetatable({ plugin = plugin, running = false, auto_scheduled = false }, self)
end
...
-- Download every not-yet-local book from the subscribed categories.
...
function Subscriptions:syncAll(opts)
    ...
    UIManager:nextTick(function()
        ...
        for _, sub in ipairs(self:list()) do
            ...
            self.last_counts[sub.name or sub.id] = n
        end
